...
int main( )
{
	...
	alt_ic_isr_register( SD_TIMER_IRQ_INTERRUPT_CONTROLLER_ID, SD_TIMER_IRQ, sdTimerInterrupt, NULL,
	        NULL );
	...
	while( 1 )
	{
		...
		{  // inicjalizujemy warstwê sprzêtow¹
		...
		if( f_open( &File, "SD.txt", FA_READ | FA_OPEN_ALWAYS ) != FR_OK )
		...
		while( 1 )
		{
			if( f_read( &File, &Buff, BUFF_SIZE, &i ) == FR_OK )
			{  // odczytujemy maksymalnie 255 znaków z pliku
		...
		if( Buff[0] == 'Y' )
		{
			if( f_open( &File, "SD.txt", FA_WRITE | FA_CREATE_ALWAYS ) != FR_OK )
			{  // jeœli chce wyczyœciæ plik to to czynimy
		...
		{
			if( f_open( &File, "SD.txt", FA_WRITE | FA_OPEN_APPEND ) != FR_OK )
			{  // a jeœli chce coœ dopisaæ na koñcu to otwieramy plik w odpowiednim trybie
				alt_putstr( "File Open for writing ERROR\r\n" );
				waitUser( );
				continue;
			}
		...
		if( f_write( &File, Buff, i, &i ) != FR_OK )
		{  // i zapisujemy go do pliku
