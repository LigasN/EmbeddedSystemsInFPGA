...
)
{
	...
	// odczytaj dane na wszelki wypadek, aby opró¿nic bufor
	IORD_ALTERA_AVALON_SPI_RXDATA(SD_SPI_BASE);

	// czekaj a¿ uk³ad bêdzie gotowy do nadawania
	...
	// czekaj a¿ zostanie odebrany jeden bajt danych (podczas wysy³ania)
	while ((IORD_ALTERA_AVALON_SPI_STATUS(SD_SPI_BASE) &
	...
	// czekaj, dopóki nie zostanie zakoñczony ca³y cykl transmisji
	while ((IORD_ALTERA_AVALON_SPI_STATUS(SD_SPI_BASE) &
